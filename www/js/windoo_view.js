...
function drawAllChart(){
    new drawChart('tempChart', 'chartTemp');
    new drawChart('humdChart', 'chartHumd');
    new drawChart('windChart', 'chartWind');
    new drawChart('presChart', 'chartPres');
}
function drawChart(target, val){
    ...
    var min = 0;
    var max = d3.max(data, function(d){ return d.y; })+1;
    ...
    var line = d3.svg.line().interpolate('monotone')
        ...
        .y(function(d) { return yScale(d.y-min); });

    var lineArea = d3.svg.area()
		.x(function(d){ return xScale(d.x); })
		.y0(yScale(min))
        .y1(function(d){ return yScale(d.y-min); })
